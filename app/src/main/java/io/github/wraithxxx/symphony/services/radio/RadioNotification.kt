        ...
        val notification = NotificationCompat.Builder(
            ...
            setContentIntent(
                PendingIntent.getActivity(
                    symphony.applicationContext,
                    0,
                    LauncherIconManager.createLaunchIntent(symphony.applicationContext),
            ...
            setOngoing(req.isPlaying || req.isPlayPending)
            addAction(
                createAction(
            ...
            addAction(
                when {
                    req.isPlaying || req.isPlayPending -> createAction(
                    R.drawable.material_icon_pause,
                    symphony.t.Pause,
                    RadioSession.ACTION_PAUSE
                    )

                    else -> createAction(
                    R.drawable.material_icon_play,
                    symphony.t.Play,
                    RadioSession.ACTION_PLAY
                    )
                }
            )
            addAction(
