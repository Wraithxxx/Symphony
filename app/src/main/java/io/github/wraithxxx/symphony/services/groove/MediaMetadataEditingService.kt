...
class MediaMetadataEditingService(private val symphony: Symphony) {
    private val pendingArtworkStore = PendingArtworkMutationStore(symphony.applicationContext)
    private val pendingArtwork = ConcurrentHashMap<String, PendingArtworkMutationStore.Record>()
    private val pendingArtworkFlushMutex = Mutex()
    private var radioUpdateUnsubscribe: EventUnsubscribeFn? = null
    private var pendingArtworkFlushJob: Job? = null
    ...
    suspend fun start() {
        if (radioUpdateUnsubscribe != null) return
        pendingArtwork.putAll(pendingArtworkStore.load().associateBy { it.songId })
        radioUpdateUnsubscribe = symphony.radio.onUpdate.subscribe {
            schedulePendingArtworkFlush()
        }
    }

    fun stop() {
        radioUpdateUnsubscribe?.invoke()
        radioUpdateUnsubscribe = null
        pendingArtworkFlushJob?.cancel()
        pendingArtworkFlushJob = null
    }

    internal fun onLibraryReady() {
    ...
    internal suspend fun applyPendingArtworkOverlays() {
        pendingArtwork.values.forEach { record ->
            val song = symphony.groove.song.get(record.songId) ?: return@forEach
            ensurePendingCoverFile(record)
            if (song.coverFile != record.coverFile) {
                symphony.groove.publishArtworkOverlay(
                    song,
                    song.copy(coverFile = record.coverFile),
                )
            }
        }
    }
    ...
    suspend fun save(songId: String, draft: Draft, edit: Edit): SaveResult =
        symphony.groove.withLibraryTransaction {
            ...
            if (!draft.tagEditingSupported) {
                return@withLibraryTransaction if (renameRequested) {
                    saveFilenameOnly(song, edit.filenameBase)
                } else {
            ...
            val propertyMutationRequested = EDITABLE_PROPERTY_KEYS.any { key ->
                MediaMetadataEditPolicy.propertyValuesDiffer(
                    draft.originalProperties[key],
                    properties[key],
                )
            }
            val artworkMutationRequested = edit.artwork !is ArtworkChange.Keep
            if (MediaArtworkMutationPolicy.shouldDefer(
                    artworkMutationRequested = artworkMutationRequested,
                    propertyMutationRequested = propertyMutationRequested,
                    hasOpenPlayerForFile = symphony.radio.hasOpenPlayerForFile(song.id),
                )
            ) {
                val targetSong = if (renameRequested) {
                    when (val rename = saveFilenameOnly(song, edit.filenameBase)) {
                        is SaveResult.Success -> rename.song
                        else -> return@withLibraryTransaction rename
                    }
                } else {
                    song
                }
                return@withLibraryTransaction deferArtworkMutation(
                    song = targetSong,
                    artwork = edit.artwork,
                )
            }
            val fileContentMutationRequested =
                artworkMutationRequested || propertyMutationRequested
            if (!fileContentMutationRequested) {
                return@withLibraryTransaction if (renameRequested) {
                    saveFilenameOnly(song, edit.filenameBase)
                } else {
                    SaveResult.Success(song)
                }
            }
            val document = try {
                withContext(Dispatchers.IO) {
    ...
    private suspend fun saveFilenameOnly(song: Song, requestedBaseName: String): SaveResult {
        return when (
            val result = symphony.groove.renaming.renameInLibraryTransaction(
                songId = song.id,
            ...
            is MediaRenameService.Result.Success -> {
                reconcileEditedSong(song, result.song)
                SaveResult.Success(result.song)
            }
            MediaRenameService.Result.NotFound -> SaveResult.NotFound
            MediaRenameService.Result.InvalidName -> SaveResult.InvalidName
            MediaRenameService.Result.Unchanged -> SaveResult.Success(song)
            MediaRenameService.Result.Conflict -> SaveResult.Conflict
            MediaRenameService.Result.PermissionDenied -> SaveResult.PermissionDenied
            MediaRenameService.Result.Unsupported -> SaveResult.RenameUnsupported
            is MediaRenameService.Result.Failed -> SaveResult.Failed(result.reason)
        }
    }

    private suspend fun deferArtworkMutation(
        song: Song,
        artwork: ArtworkChange,
    ): SaveResult {
        if (artwork is ArtworkChange.Keep) return SaveResult.Success(song)
        val coverFile = when (artwork) {
            ArtworkChange.Remove -> null
            is ArtworkChange.Replace -> {
                val format = AudioArtwork.Format.fromMimeType(artwork.mimeType)
                val extension = format.extension.ifEmpty { "image" }
                "${song.id}-pending-${System.currentTimeMillis()}.$extension"
            }
            ArtworkChange.Keep -> null
        }
        val record = try {
            pendingArtworkStore.put(song, coverFile, artwork)
        } catch (error: Exception) {
            Logger.error("MediaMetadataEditingService", "pending artwork save failed", error)
            return SaveResult.Failed(error.localizedMessage ?: error.toString())
        }
        if (artwork is ArtworkChange.Replace && coverFile != null) {
            try {
                withContext(Dispatchers.IO) {
                    FileOutputStream(symphony.database.artworkCache.get(coverFile)).use {
                        it.write(artwork.bytes)
                    }
                }
            } catch (error: Exception) {
                pendingArtworkStore.remove(record)
                Logger.error("MediaMetadataEditingService", "live artwork cache failed", error)
                return SaveResult.Failed(error.localizedMessage ?: error.toString())
            }
        }
        pendingArtwork[song.id] = record
        song.coverFile
            ?.takeIf { it != coverFile }
            ?.let { oldCover ->
                runCatching { symphony.database.artworkCache.get(oldCover).delete() }
            }
        val updatedSong = song.copy(coverFile = coverFile)
        reconcileEditedSong(song, updatedSong)
        schedulePendingArtworkFlush()
        return SaveResult.Success(updatedSong)
    }

    private fun schedulePendingArtworkFlush() {
        if (pendingArtwork.isEmpty() || pendingArtworkFlushJob?.isActive == true) return
        pendingArtworkFlushJob = symphony.groove.coroutineScope.launch {
            pendingArtworkFlushMutex.withLock {
                symphony.groove.withLibraryTransaction {
                    flushPendingArtwork()
                }
            }
    ...
    private suspend fun flushPendingArtwork() {
        pendingArtwork.values.toList().forEach { record ->
            if (symphony.radio.hasOpenPlayerForFile(record.songId)) return@forEach
            val song = symphony.groove.song.get(record.songId) ?: return@forEach
            if (!writePendingArtwork(song, record)) return@forEach
            if (!pendingArtwork.remove(record.songId, record)) return@forEach
            pendingArtworkStore.remove(record)
            val document = withContext(Dispatchers.IO) {
                DocumentFileX.fromSingleUri(symphony.applicationContext, song.uri)
            }
            val committedSong = document?.let {
                song.copy(dateModified = it.lastModified, size = it.size)
            } ?: song
            reconcileEditedSong(song, committedSong)
        }
    }

    private suspend fun writePendingArtwork(
        song: Song,
        record: PendingArtworkMutationStore.Record,
    ): Boolean {
        val payload = when (record.kind) {
            PendingArtworkMutationStore.Kind.Replace ->
                pendingArtworkStore.readPayload(record) ?: return false
            PendingArtworkMutationStore.Kind.Remove -> null
        }
        val pictures = when (record.kind) {
            PendingArtworkMutationStore.Kind.Replace -> arrayOf(
                Picture(
                    data = payload!!,
                    description = "",
                    pictureType = "Front Cover",
            ...
            PendingArtworkMutationStore.Kind.Remove -> emptyArray()
        }
        return try {
            StorageMutationRetry.run(shouldRetry = { successful -> !successful }) { attempt ->
                val successful = try {
                    withContext(Dispatchers.IO) {
                        withWriteDescriptor(song.uri) { fd ->
                            TagLib.savePictures(fd, pictures)
                        } && verifyPendingArtwork(song.uri, record.kind)
                    }
                } catch (error: Exception) {
                    Logger.warn(
                        "MediaMetadataEditingService",
                        "deferred artwork write failed on attempt $attempt",
                        error,
                    )
                    false
                }
            ...
            Logger.error("MediaMetadataEditingService", "deferred artwork commit failed", error)
            false
        }
    }

    private fun verifyPendingArtwork(
        uri: Uri,
        kind: PendingArtworkMutationStore.Kind,
    ): Boolean {
        val metadata = withReadDescriptor(uri) { fd ->
            TagLib.getMetadata(fd, readPictures = true)
        } ?: return false
        return when (kind) {
            PendingArtworkMutationStore.Kind.Replace -> metadata.pictures.isNotEmpty()
            PendingArtworkMutationStore.Kind.Remove -> metadata.pictures.isEmpty()
        }
    }

    private suspend fun ensurePendingCoverFile(record: PendingArtworkMutationStore.Record) {
        val coverFile = record.coverFile ?: return
        val target = symphony.database.artworkCache.get(coverFile)
        if (target.isFile) return
        val payload = pendingArtworkStore.readPayload(record) ?: return
        withContext(Dispatchers.IO) {
            FileOutputStream(target).use { it.write(payload) }
        }
    }
    ...
    private suspend fun reconcileEditedSong(originalSong: Song, updatedSong: Song) {
        ...
        runCatching {
            ...
            Logger.error("MediaMetadataEditingService", "edited song persistence failed", it)
        }
    }
    ...
    companion object {
        ...
        private val EDITABLE_PROPERTY_KEYS = listOf(
            ARTIST,
            ALBUM,
            ALBUM_ARTIST,
            COMPOSER,
