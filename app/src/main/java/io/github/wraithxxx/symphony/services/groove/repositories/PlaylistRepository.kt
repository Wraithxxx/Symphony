...
class PlaylistRepository(private val symphony: Symphony) {
    ...
    @Volatile
    private var cache = ConcurrentHashMap<String, Playlist>()
    ...
    suspend fun fetch() {
        ...
        try {
            ...
            val nextCache = ConcurrentHashMap<String, Playlist>()
            ...
            playlists.values.forEach { x ->
                ...
                nextCache[playlist.id] = playlist
                nextIds.add(playlist.id)
            }
            if (!nextCache.containsKey(FAVORITE_PLAYLIST)) {
                val favorites = create(FAVORITE_PLAYLIST, "Favorites", emptyList())
                nextCache[favorites.id] = favorites
                nextIds.add(favorites.id)
                symphony.database.playlists.insert(favorites)
            }
            cache = nextCache
            _all.value = nextIds.distinct()
            emitCount()
    ...
    internal suspend fun removeSongPaths(songPaths: Set<String>) {
        if (songPaths.isEmpty()) {
            return
        }
        val affected = cache.values.filter {
            it.isNotLocal && it.songPaths.any(songPaths::contains)
        }
        affected.forEach { playlist ->
            val updated = playlist.copy(
                songPaths = playlist.songPaths.filterNot(songPaths::contains),
            )
            cache[playlist.id] = updated
            symphony.database.playlists.update(updated)
        }
        if (affected.isNotEmpty()) {
            _favorites.value = getFavorites().getSongIds(symphony)
            emitUpdateId()
        }
    }

    internal suspend fun replaceSongPath(oldPath: String, newPath: String) {
        if (oldPath == newPath) {
            return
        }
        val affected = cache.values.filter {
            it.isNotLocal && oldPath in it.songPaths
        }
        affected.forEach { playlist ->
            val updated = playlist.copy(
                songPaths = playlist.songPaths.map { path ->
                    if (path == oldPath) newPath else path
                },
            )
            cache[playlist.id] = updated
            symphony.database.playlists.update(updated)
