package io.github.wraithxxx.symphony.ui.view.settings
...
import io.github.wraithxxx.symphony.ui.components.IconButtonPlaceholder
import io.github.wraithxxx.symphony.ui.components.ConfirmationDialog
import io.github.wraithxxx.symphony.ui.components.TopAppBarMinimalTitle
import io.github.wraithxxx.symphony.ui.components.settings.ConsiderContributingTile
import io.github.wraithxxx.symphony.ui.components.settings.SettingsOptionTile
import io.github.wraithxxx.symphony.ui.components.settings.SettingsSideHeading
import io.github.wraithxxx.symphony.ui.components.settings.SettingsSimpleTile
import io.github.wraithxxx.symphony.ui.components.settings.SettingsSliderTile
import io.github.wraithxxx.symphony.ui.components.settings.SettingsSwitchTile
...
fun PlayerSettingsView(context: ViewContext) {
    ...
    val audioInterruptionBehavior by
        context.symphony.settings.audioInterruptionBehavior.flow.collectAsState()
    ...
    val rememberTrackPositions by
        context.symphony.settings.rememberTrackPositions.flow.collectAsState()
    val minimumRememberedTrackDurationMinutes by
        context.symphony.settings.minimumRememberedTrackDurationMinutes.flow.collectAsState()
    val rememberedPositions by context.symphony.radio.progress.entries.collectAsState()
    var showClearRememberedPositionsDialog by remember { mutableStateOf(false) }
            ...
            Box(
                ...
                Column(modifier = Modifier.verticalScroll(scrollState)) {
                    ...
                    SettingsOptionTile(
                        ...
                        value = audioInterruptionBehavior,
                        values = linkedMapOf(
                            AudioInterruptionBehavior.PauseAndResume to
                                    "Pause and resume automatically",
                            AudioInterruptionBehavior.KeepPlaying to "Keep playing",
                        ),
                        captions = mapOf(
                            AudioInterruptionBehavior.PauseAndResume to
                                    "Respect other apps and resume after temporary interruptions.",
                            AudioInterruptionBehavior.KeepPlaying to
                                    "Continue during other audio. Sounds may overlap.",
                        ),
                        onChange = { value ->
                            context.symphony.settings.audioInterruptionBehavior.setValue(value)
                        }
                    )
                    HorizontalDivider()
                    SettingsSwitchTile(
                    ...
                    SettingsSideHeading("Playback progress")
                    SettingsSwitchTile(
                        icon = {
                            Icon(Icons.Filled.History, null)
                        },
                        title = {
                            Text("Remember track positions")
                        },
                        subtitle = {
                            Text(
                                when {
                                    rememberTrackPositions ->
                                        "Resume tracks at least " +
                                                "$minimumRememberedTrackDurationMinutes minutes long"
                                    else ->
                                        "Tracks normally start from the beginning when reopened"
                                }
                            )
                        },
                        value = rememberTrackPositions,
                        onChange = { value ->
                            context.symphony.settings.rememberTrackPositions.setValue(value)
                        },
                    )
                    AnimatedVisibility(rememberTrackPositions) {
                        Column {
                            HorizontalDivider()
                            SettingsSliderTile(
                                context,
                                ...
                                label = { value ->
                                    Text("${value.toInt()} minutes")
                                },
                                range = 5f..180f,
                                initialValue = minimumRememberedTrackDurationMinutes.toFloat(),
                                onValue = { value ->
                                    (value.roundToInt() / 5 * 5)
                                        .coerceIn(5, 180)
                                        .toFloat()
                                },
                                onChange = { value ->
                                    context.symphony.settings
                                        .minimumRememberedTrackDurationMinutes
                            ...
                            SettingsSimpleTile(
                                ...
                                title = {
                                    Text("Clear remembered positions")
                                },
                                subtitle = {
                                    Text(
                                        when (rememberedPositions.size) {
                                            1 -> "1 track"
                                            else -> "${rememberedPositions.size} tracks"
                                        }
                                    )
                                },
                                onClick = {
                                    if (rememberedPositions.isNotEmpty()) {
                                        showClearRememberedPositionsDialog = true
                                    }
                                },
                            )
                            Text(
                                "Current playback may still be restored after an app or " +
                                        "service restart.",
    ...
    if (showClearRememberedPositionsDialog) {
        ConfirmationDialog(
            context,
            title = {
                Text("Clear remembered positions?")
            ...
            description = {
                Text(
                    "Clear remembered positions for ${rememberedPositions.size} tracks? " +
                            "Audio files and the current playback position will not be removed."
                )
            },
            onResult = { confirmed ->
                showClearRememberedPositionsDialog = false
                if (confirmed) {
                    context.symphony.radio.progress.clear()
                }
            },
        )
    }
