...
class AlbumRepository(private val symphony: Symphony) {
    ...
    @Volatile
    private var cache = ConcurrentHashMap<String, Album>()
    @Volatile
    ...
    internal fun replaceAll(songs: Collection<Song>) {
        val replacement = AlbumRepository(symphony)
