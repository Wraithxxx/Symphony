...
import io.github.wraithxxx.symphony.utils.concurrentListOf
...
class RadioQueue(private val symphony: Symphony) {
    ...
    var currentLoopMode = symphony.settings.lastLoopMode.value
        private set(value) {
            if (field == value) {
                return
            }
            field = value
            symphony.settings.lastLoopMode.setValue(value)
    ...
    fun remove(index: Int, playReplacement: Boolean = true) {
        originalQueue.removeAt(index)
        currentQueue.removeAt(index)
        symphony.radio.onUpdate.dispatch(Radio.Events.Queue.Modified)
        if (currentSongIndex == index && playReplacement) {
    ...
    internal fun reconcileLibrary(librarySongIds: Set<String>) {
        val result = RadioQueueLibraryReconciler.reconcile(
            ...
            currentQueue = currentQueue.toList(),
            currentSongIndex = currentSongIndex,
            librarySongIds = librarySongIds,
        )
        if (
            result.originalQueue == originalQueue.toList() &&
            result.currentQueue == currentQueue.toList() &&
            result.currentSongIndex == currentSongIndex
        ) {
            return
        }

        originalQueue.clear()
        originalQueue.addAll(result.originalQueue)
        currentQueue.clear()
        currentQueue.addAll(result.currentQueue)
        currentSongIndex = result.currentSongIndex
        symphony.radio.onUpdate.dispatch(Radio.Events.Queue.Modified)
    }

    internal fun removeDeletedSong(songId: String): RadioQueueSongRemovalPlanner.Result {
        return removeDeletedSongs(setOf(songId))
    }

    internal fun removeDeletedSongs(songIds: Set<String>): RadioQueueSongRemovalPlanner.Result {
