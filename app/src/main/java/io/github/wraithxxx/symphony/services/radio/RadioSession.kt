...
class RadioSession(val symphony: Symphony) {
    ...
    fun handleAction(action: String) {
        when (action) {
            ...
            ACTION_PLAY -> symphony.radio.shorty.play()
            ACTION_PAUSE -> symphony.radio.shorty.pause()
    ...
    private fun update() {
        ...
        publishCoreSession(generation, snapshot)
    ...
    private suspend fun updateAsync(
        ...
        synchronized(publicationLock) {
            if (
                ...
                !publicationState.allowsAsyncPublication(generation) ||
    ...
    private fun publishCoreSession(generation: Long, snapshot: RadioPlaybackSnapshot) {
        synchronized(publicationLock) {
            if (!publicationState.beginCorePublication(generation)) {
                return
            }
            ensureEnabled()
    ...
    companion object {
        ...
        val ACTION_PLAY = "${R.string.app_name}_play"
        val ACTION_PAUSE = "${R.string.app_name}_pause"
